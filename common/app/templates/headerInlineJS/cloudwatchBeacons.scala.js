    ...
    require([
        ...
        (function (window, navigator) {
            function logDevice(model, device) {
                var isCoreIpadMVTUser = function() {
                    var participations = ab.getParticipations().filter(function(abTest) {
                        return abTest.id === 'ipad-core-fronts' && abTest.canRun();
                    });
                    return participations.length > 0;
                };

                var identifier = function() {
                  var baseId = device + '-' + model;
                  return (device === 'ipad' && isCoreIpadMVTUser()) ? baseId + '-mvt-core-fronts' : baseId;
                };
