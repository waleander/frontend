...
/**
  ...
  */
object MyCSRFAddToken {

  private class MyCSRFAddTokenAction[A](config: CSRFConfig, tokenProvider: TokenProvider, wrapped: Action[A]) extends Action[A] {
    println("IN ADD TOKEN ACTION")
    def parser = wrapped.parser
    def apply(request: Request[A]) = {
      if (CSRFAction.getTokenFromHeader(request, config).isEmpty) {
        // No token in header and we have to create one if not found, so create a new token
        println("MAKING NEW TOKEN")
        val newToken = tokenProvider.generateToken
  ...
  def apply[A](action: Action[A], config: CSRFConfig = CSRFConfig.global): Action[A] =
    new MyCSRFAddTokenAction(config, new TokenProviderProvider(config).get, action)
}
